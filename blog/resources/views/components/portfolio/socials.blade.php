<div class="mt-10 px-4">
    <h3 class="text-xl font-bold text-gray-800 dark:text-white mb-4 text-white text-center">Socials</h3>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-center">
        @isset($entity)
        @foreach($entity as $social)
        <div data-aos="fade-up" style="" data-aos-anchor-placement="top-center" class="dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden mb-6 p-6 transition-shadow duration-300 hover:shadow-xl">
            <a target="_blank" href="{{ $social->link }}" class="block hover:underline hover:text-blue-500">
                <img class="h-45 w-full object-cover rounded-lg mt-2 mx-auto" src="{{ asset('storage/' . $social->image) }}" alt="Social Image">
